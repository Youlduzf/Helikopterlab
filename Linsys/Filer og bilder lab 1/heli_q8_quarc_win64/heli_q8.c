 ...
 * C source code generated on : Mon Aug 30 13:14:33 2021
 *
...
void heli_q8_output0(void)             /* Sample time: [0.0s, 0.0s] */
{
  ...
  if (rtmIsMajorTimeStep(heli_q8_M)) {
     ...
     */
    heli_q8_B.Add = heli_q8_P.Constant_Value + heli_q8_B.ElevationCounttorad;
     ...
     *  Sum: '<S2>/Sum'
     */
    rtb_DeadZonex = heli_q8_B.Joystick_gain_y - heli_q8_B.Add;
   ...
   *  Sum: '<S6>/Sum'
   */
  rtb_Add_g = ((heli_q8_X.Integrator_CSTATE + heli_q8_B.K_ep) -
  ...
  if (rtb_Backgain < heli_q8_P.Step_Time) {
  ...
  /* Step: '<Root>/Step1' */
  if (rtb_Backgain < heli_q8_P.Step1_Time) {
   ...
   *  Sum: '<S5>/Sum1'
   */
  rtb_Backgain = ((u0 - rtb_Backgain) - heli_q8_B.PitchCounttorad) *
    heli_q8_P.kp - heli_q8_P.kd * heli_q8_B.PitchTransferFcn;
  if (rtmIsMajorTimeStep(heli_q8_M)) {
    /* Gain: '<S6>/K_ei' */
      ...
      if (rtmIsMajorTimeStep(heli_q8_M) ) {
        {
          if (!(++heli_q8_DW.ToFile_IWORK.Decimation % 1) &&
              (heli_q8_DW.ToFile_IWORK.Count * (1 + 1)) + 1 < 100000000 ) {
            ...
            if (fp != (NULL)) {
              ...
              if (fwrite(u, sizeof(real_T), 1 + 1, fp) != 1 + 1) {
                rtmSetErrorStatus(heli_q8_M,
                                  "Error writing to MAT-file (-1,-1).mat");
                return;
              }

              if (((++heli_q8_DW.ToFile_IWORK.Count) * (1 + 1))+1 >= 100000000)
              {
                (void)fprintf(stdout,
                              "*** The ToFile block will stop logging data before\n"
                              "    the simulation has ended, because it has reached\n"
                              "    the maximum number of elements (100000000)\n"
                              "    allowed in MAT-file (-1,-1).mat.\n");
              }
            }
          }
        }
   ...
   *  Sum: '<S1>/Add'
   */
  u0 = (rtb_Add_g - rtb_Backgain) * heli_q8_P.Frontgain_Gain;
   ...
   *  Sum: '<S1>/Subtract'
   */
  u0 = (rtb_Add_g + rtb_Backgain) * heli_q8_P.Backgain_Gain;
  ...
  /* Sum: '<S3>/Add1' */
  heli_q8_B.Add1 = heli_q8_B.FrontmotorSaturation +
...
void heli_q8_initialize(void)
{
  ...
  {
    FILE *fp = (NULL);
    char fileName[509] = "(-1,-1).mat";
    if ((fp = fopen(fileName, "wb")) == (NULL)) {
      rtmSetErrorStatus(heli_q8_M, "Error creating .mat file (-1,-1).mat");
      return;
    }

    if (rt_WriteMat4FileHeader(fp, 1 + 1, 0, "ans")) {
      rtmSetErrorStatus(heli_q8_M,
                        "Error writing mat file header to file (-1,-1).mat");
      return;
    }
...
void heli_q8_terminate(void)
{
  ...
  /* Terminate for ToFile: '<S3>/To File' */
  {
    ...
    if (fp != (NULL)) {
      char fileName[509] = "(-1,-1).mat";
      if (fclose(fp) == EOF) {
        rtmSetErrorStatus(heli_q8_M, "Error closing MAT-file (-1,-1).mat");
        return;
      }

      if ((fp = fopen(fileName, "r+b")) == (NULL)) {
        rtmSetErrorStatus(heli_q8_M, "Error reopening MAT-file (-1,-1).mat");
        return;
      }

      if (rt_WriteMat4FileHeader(fp, 1 + 1, heli_q8_DW.ToFile_IWORK.Count, "ans"))
      {
        rtmSetErrorStatus(heli_q8_M,
                          "Error writing header for ans to MAT-file (-1,-1).mat");
      }
...
RT_MODEL_heli_q8_T *heli_q8(void)
{
  ...
  /* External mode info */
  heli_q8_M->Sizes.checksums[0] = (1347659043U);
  heli_q8_M->Sizes.checksums[1] = (2798919884U);
  heli_q8_M->Sizes.checksums[2] = (191130007U);
  heli_q8_M->Sizes.checksums[3] = (2748211473U);
